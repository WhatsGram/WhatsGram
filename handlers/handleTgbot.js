...
const handleTgBot = async (ctx , client , MessageMedia) => {
  const sendMsgToWa = async (msg, chatId, msgId) => {
    if(!msg.text && chatId){
      ...
      client.sendMessage(chatId, msg.text.startsWith('/send') ? msg.text.split('|')[2].trim() : msg.text.trim(), { quotedMessageId: msgId ? msgId : null });
    }
  }
  ...
  if(ctx.message.from.id == TG_OWNER_ID){
    if (
       ...
       if(ctx.message.text === '/mar') {
          ...
          sendMsgToWa(ctx.message.reply_to_message, chatId)  
        }else { sendMsgToWa(ctx.message, getIds().waChatId, getIds().waMessageId) }
